...
def visualize(CATEGORIES, test_image_paths, test_labels_ids, predicted_categories_ids, train_labels_paths,
              train_labels_ids):
    ...
    for k, name in enumerate(CATEGORIES):
        ...
        train_path = os.path.relpath(os.path.join(thumbnails_path, name + '_train_' + os.path.basename(Train_name[k])),
                                     results_path)
        tp_path = os.path.relpath(os.path.join(thumbnails_path, name + '_TP_' + os.path.basename(TP_name[k])),
                                  results_path)
        fp_path = os.path.relpath(os.path.join(thumbnails_path, name + '_FP_' + os.path.basename(FP_name[k])),
                                  results_path)
        fn_path = os.path.relpath(os.path.join(thumbnails_path, name + '_FN_' + os.path.basename(FN_name[k])),
                                  results_path)
        panel.write(
            '| ' + name + ' | ' + '![](' + train_path + ')' + ' | ' + '![](' + tp_path + ')' + ' | ' + '![](' + fp_path + ')' + ' | ' + '![](' + fn_path + ')' + ' |' + '\n')
